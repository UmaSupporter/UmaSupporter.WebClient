...
const CardListView: React.FC<Props> = (props: Props) => {
  ...
  const toggleFilter = (cardType: CARD_TYPE) =>
    dispatch({ type: 'TOGGLE_FILTER', payload: cardType });
  const currentCardType = Object.values(CARD_TYPE).map((x) => {
    return {
  ...
  return (
    <div className={'CardList'}>
      ...
      <div className={'MainPageCardList'}>
        <p className={'MainPagelabel'}>카드 리스트</p>
      </div>
      <CardTypeFilterComponent
        cardTypes={currentCardType}
        onClickType={toggleFilter}
      />
